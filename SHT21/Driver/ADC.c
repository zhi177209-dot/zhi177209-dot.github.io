...
//ADC_CONTR 电源控制
#define ADC_POWER_ON   (1<<7)
//ADC_CONTR 时钟设置
#define ADC_CLK_540T   (0<<5)
#define ADC_CLK_360T   (1<<5)
#define ADC_CLK_180T   (2<<5)
#define ADC_CLK_90T    (3<<5)
//========================================================================
...
u16	Get_ADC10bitResult(u8 channel)	//channel = 0~7
{
	...
	ADC_CONTR=ADC_POWER_ON|ADC_CLK_540T|ADC_START|channel;//启动通道ch的A/转换
  _nop_();
	_nop_();
	_nop_();
	_nop_();//等待四个时钟周期
	while((ADC_CONTR&ADC_FLAG)!=ADC_FLAG);//如果A/D转换还未结束，则一直等
	ADC_CONTR&=~ADC_FLAG;//如果已经转换完成，则清零转换完成标志	
	return ((uint16_t)ADC_RES<<2)+ADC_RESL;//合并成10位的结果并返回;	
